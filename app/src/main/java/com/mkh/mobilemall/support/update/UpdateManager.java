package com.mkh.mobilemall.support.update;
...
import com.mkh.mobilemall.dao.SettingDao;
...
public class UpdateManager {
                ...
                if (msg.what == 1) {
                    ...
                    if (mUpdate != null) {
                        if (curVersionCode < mUpdate.getVersioncode()) {
                            ...
                            showNoticeDialog(mUpdate.getIsForce());
                        } else if (isShowMsg) {
     ...
     * 显示版本更新通知对话框
     */
    public void showNoticeDialog(final String isForce) {

        final AlertDialog alg = new Builder(mContext).create();
        ...
        LinearLayout llyUpdate= (LinearLayout) window.findViewById(R.id.llyoutupdate);
        LinearLayout llyMustUpdate= (LinearLayout) window.findViewById(R.id.llyoutmustupdate);
        TextView txtMustBe= (TextView) window.findViewById(R.id.dialog_button_entry_must);

        TextView btnEntry = (TextView) window
                  .findViewById(R.id.dialog_button_entry);
        final TextView btnCancle = (TextView) window
        ...
        if (isForce.equals("true")) {
            llyUpdate.setVisibility(View.GONE);
            llyMustUpdate.setVisibility(View.VISIBLE);
        ...
        txtMustBe.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                alg.dismiss();
                showDownloadDialog(isForce);
     ...
     * 显示下载对话框
     */
    public void showDownloadDialog(final String isForce) {
        ...
        mProgressText = (TextView) window.findViewById(R.id.update_progress_text);
    ...
    private Runnable mdownApkRunnable = new Runnable() {
        ...
        public void run() {
            try {
                String apkName = "mkh" + mUpdate.getVersionname()
                          + ".apk";
                String tmpApk = "mkh" + mUpdate.getVersionname()
                          + ".tmp";
                // 判断是否挂载了SD卡
                String storageState = Environment.getExternalStorageState();
                if (storageState.equals(Environment.MEDIA_MOUNTED)) {
                    savePath = Environment.getExternalStorageDirectory()
                              .getAbsolutePath() + "/mkh/Update/";
